...
> **⚠️ STATUS (2026-06-17): Auth is SHIPPED — do NOT defer, strip, or rebuild it.** Apple + Google Sign In, the identity screen, and the data-preserving migration gate merged to `main` (PR #14); the 21+ age gate + signup logging shipped in PR #16. The "DEFERRED to v1.1" markers in Phases 2/4/5 below are **historical** — leave the shipped auth alone.
>
> **⚠️ TAT-16 reviewer "dev mode" REMOVED (2026-07-06): do NOT rebuild it.** Apple rejected build 18 under Guideline 2.2 for the dev-mode login — beta-testing features can't ship in production. It was also unnecessary: App Review signs in with their own Apple ID (Sign in with Apple needs no demo account) and completes the $24.99 unlock in the StoreKit sandbox. `src/config/reviewerAccess.ts` and `ReviewerAccessMenu` are deleted; App Store Connect review notes now say "sign in with any Apple ID," with "Sign-in required" unchecked. The same rejection required in-app account deletion (Guideline 5.1.1(v)) — "Erase Everything" became **Delete Account & Data** (local wipe + Google sign-out + best-effort server-side signup-record deletion via the `action: 'delete'` webhook in `functions/signup-email`).
>
