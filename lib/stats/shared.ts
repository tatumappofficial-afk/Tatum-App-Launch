...
export function topActivities(encounters: Encounter[], tags: ActivityTag[], limit: number): ActivityCount[] {
 ...
 */
export function topPartners(encounters: Encounter[], partners: Partner[], limit?: number): PartnerCount[] {
  const counts = new Map<string, number>()
  for (const enc of encounters) {
 ...
 */
export function dayOfWeekHistogram(encounters: Encounter[], calendarStartDay: CalendarStartDay): number[] {
 ...
 */
export function partnerPeriodStats(windowEncounters: Encounter[], partners: Partner[]): PartnerLifetimeStats[] {
  const partnerById = new Map(partners.map((p) => [p.id, p]))
  ...
  for (const [id, pEnc] of buckets) {
    ...
    const averageStars = rated.length === 0 ? null : rated.reduce((s, e) => s + (e.stars ?? 0), 0) / rated.length
    const counts = new Map<string, number>()
  ...
  rows.sort((a, b) => b.sessionCount - a.sessionCount || a.partner.displayName.localeCompare(b.partner.displayName))
  return rows
}
 ...
 */
export function partnerLifetimeStats(allEncounters: Encounter[], partners: Partner[]): PartnerLifetimeStats[] {
  return partners
    .filter((p) => p.isActive)
    .map((p) => {
...
export function distinctWeekdays(encounters: Encounter[], calendarStartDay: CalendarStartDay): string[] {
  const seen = new Set<number>()
  for (const enc of encounters) {
    seen.add(parseDateString(enc.date).getDay())
  }
  const order = calendarStartDay === 'sunday' ? [0, 1, 2, 3, 4, 5, 6] : [1, 2, 3, 4, 5, 6, 0]
